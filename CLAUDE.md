...
- Handles IPC messages for page navigation
- Delegates menu/file handling to MenuController

### Menu System (menu.js)
- MenuController handles File > Open dialog for quiz files
- Validates quiz data based on file extension
...
- Routes to appropriate quiz view based on extension
...
- **Format**: Object with `upperLabel`, `lowerLabel`, and `items` array (1-11 strings)
- **Extension**: `.sortquiz`
...
- **Gameplay**:
  ...
  - Preview item shows where it will be placed (yellow highlight)
  - Items must be placed in ascending order (by original index)
  - Wrong placement ends game
  - Dynamic graph rebuilds after each placement: maintains spots between all items
  - Dynamic scaling reduces font/spacing when many items present
...
- **Gameplay**:
  - Shows images sequentially on Enter press
  - After all images shown, shows each image with answer
...
├── menu.js                    # MenuController - file loading and validation
...
│   ├── topic.html/css/js     # Grid selector
│   ├── question.html/css/js  # Question display
│   └── answer.html/css/js    # Answer display
├── pairquiz/                 # Pair matching quiz
│   └── pair.html/css/js      # Single page with three columns
├── sortquiz/                 # Sorting quiz
│   └── sort.html/css/js      # Single page with vertical graph
└── imagequiz/                # Image identification quiz
    └── image.html/css/js     # Single page with sequential images
```
