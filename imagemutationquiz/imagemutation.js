/**
 * Image Mutation Quiz Game Logic
 * Displays mutated images sequentially and shows comparison with original
 * Simple navigation with Enter key
 */

class ImageMutationController {
    constructor() {
        this.imageData = [];
        this.currentIndex = 0;
        ...
        if (typeof soundManager !== 'undefined') {
            console.log('Sound manager is available for image mutation quiz');
        } else {
    ...
    showCurrentImage() {
        ...
        const mutatedImagePath = this.getImagePath(currentItem.mutatedImage);
        
        this.imageDisplay.innerHTML = `<img src="${mutatedImagePath}" alt="Mutated Image">`;
    ...
    showImageWithAnswer() {
        const currentItem = this.imageData[this.currentIndex];
        const mutatedImagePath = this.getImagePath(currentItem.mutatedImage);
        const originalImagePath = this.getImagePath(currentItem.originalImage);
        
        // Clear the background image display when showing answer
        ...
        this.answerDisplay.innerHTML = `
            <div class="image-comparison">
                <div class="image-wrapper">
                    <div class="image-label">Mutated</div>
                    <img src="${mutatedImagePath}" alt="Mutated Image">
                </div>
                <div class="image-wrapper">
    ...
    getImagePath(imageFileName) {
        // Get the directory of the original .imagemutationquiz file
        const path = require('path');
        ...
        try {
            if (this.originalFilePath) {
                ...
                return path.join(__dirname, '..', 'imagemutationquiz', imageFileName);
            }
        } catch (error) {
...
// Initialize the image mutation quiz when the page loads
document.addEventListener('DOMContentLoaded', () => {
    new ImageMutationController();
});
